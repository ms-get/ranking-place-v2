  ...
  return (
    <aside className="max-w-[1140px] w-full px-3 xl:px-2">
      ...
      <div className="flex flex-wrap xl:flex-col gap-4 justify-start items-center">
        {recommendPost(lang).map(({ title, description, lastModified, image }) => (
          <article className="max-w-full w-[260px] xl:w-[560px] flex flex-col xl:flex-row gap-4" key={lastModified}>
            <Image
              className=" bg-black  h-[326px] sm:h-[200px] w-[350px] max-[350px] sm:w-[180px] xl:w-full object-cover rounded-sm"
              src={image}
              width={260}
              height={326}
              alt="추천 아티클 이미지"
